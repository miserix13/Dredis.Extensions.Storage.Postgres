...
- `Dredis.Extensions.Storage.Postgres\PostgresKeyValueStore.Probabilistic.cs` implements the probabilistic slice, including Bloom, Cuckoo, HyperLogLog, TDigest, and TopK operations.
- `Dredis.Extensions.Storage.Postgres\PostgresKeyValueStore.TimeSeries.cs` implements the time-series slice, including create/add/increment, duplicate-policy handling, retention trimming, range aggregation, metadata queries, and label-filtered multi-range reads.
- The store depends directly on `Dredis.Abstractions.Storage` for the upstream contract and `Npgsql` for PostgreSQL access. Storage now uses one parent key table with `kind`, `value`, and `expires_at`, plus child tables for hash fields, list items, set members, sorted-set members, stream data, and time-series samples; JSON, vectors, probabilistic structures, and time-series metadata stay in the parent-row `value` payload and streams store their last-generated id there.
- `Dredis.Extensions.Storage.Postgres.Tests\PostgresKeyValueStoreTests.cs` mixes lightweight contract tests with PostgreSQL integration coverage for strings, hashes, lists, sets, sorted sets, JSON, streams, vectors, probabilistic structures, and time series. The integration path uses the `DREDIS_POSTGRES_TEST_CONNECTION_STRING` environment variable.
...
- Probabilistic structures are currently implemented with exact persisted JSON state in the parent row rather than approximate PostgreSQL-native sketches. Preserve the current status semantics and deterministic ordering/serialization unless the implementation is intentionally redesigned.
- Time series store retention, duplicate policy, and labels in the parent-row payload while keeping samples in a child table keyed by timestamp. Preserve Redis-style `BLOCK` as the default duplicate policy, `TS.INCRBY` max-timestamp semantics, and exact-label filtering for `TimeSeriesMultiRangeAsync` if you optimize later.
